...
| `run:tapa_forzada` | A la trampilla del molino le forzaron el herraje, y no del lado de adentro | `a1_molino.subir_por_la_rueda` `[Explorador]` | **Todavía nadie, y ahora con el número exacto.** Es el hermano de `run:la_soga_cortada` y le falta su variante de narrador en `c1_cuerpo`. La hermana mide **28 palabras** y el presupuesto de prosa quedó en **16.726 sobre un tope de 16.726**, o sea **cero de margen** (fase «objetivos», tarea 5: las últimas catorce se las llevó el desenlace de `a1_plaza.bajar_al_rio`, que cerraba el acto en silencio). Para escribirla hay que liberar 28 palabras antes |
...
| `run:dravos_sabe` | Sabe que vos sabés, y el trato ya se está cerrando | `a1_ronda.onEnter`, fallos con Berta o Dravos, **las cuatro vías lentas de `c2_orilla`**, `c2_vado_crecido.entregar` y `.atar_lo_que_llevas_a_la_cadena`, `c2_otra_orilla.esperar_a_que_pase_la_ronda`, `cl_molino.escuchar` fallo | **Solo `cl_molino`**, y las dos lecturas se evalúan antes de elegir: la variante del tercer párrafo y el `requires: { not: … }` de `interrumpir_antes_de_que_firmen`. Ni `cl_dravos`, ni `cl_halvar`, ni `cl_desenlace`, ni los cuatro epílogos lo miran (verificado en la Fase H; la versión anterior de esta fila decía que sí). **Su ausencia abre `cl_molino.interrumpir_antes_de_que_firmen`**, la opción que solo ve quien cruzó el vado de frente |
...
- **Mago** (Saber, Debilidad `fisico`): `p_vado_oculto.nombrar_el_agua` (cruza sin Herida) · `a2_fuera_sello.escuchar_el_sello` (párrafo exclusivo: la piedra contesta) · `cl_dravos.apagar_la_runa_un_latido` (cierra el encuentro sin pelear, con 1 Herida y `agotado`). **El precio en Herida se lo puso la Fase H**: `agotado` es tag `magia` y en el clímax entero no queda ni una tirada con ese tag, así que la opción cerraba el encuentro gratis mientras su hermana de escena, `trabar_el_eje`, costaba una Herida y ni siquiera cerraba.
